            ...
            build.onEnd(result => {
              ...
              if (result.outputFiles?.length) {
                ...
                for (const ofile of result.outputFiles) {
                  ...
                  if (!/index\.css/i.test(ofile.path)) {
...
if (typeof require === 'function' && typeof module !== 'undefined' && typeof __filename === 'string') {
  // node run
  module.exports.buildAll = buildAll;

  if (require?.main?.filename === __filename && require.main.filename.endsWith('build.js')) {
    // build.js started explicitly
    buildAll();
  }
}